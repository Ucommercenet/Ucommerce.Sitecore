...
task CleanSolution -description "Cleans the complete solution" {
    Push-Location "$src"
    Exec { msbuild "$solution_file" /p:Configuration=$configuration /p:VisualStudioVersion=16 /t:Clean /verbosity:quiet }
    Pop-Location
}
...
task Compile -description "Compiles the complete solution" -depends UpdateAssemblyInfo {
    ...
    Exec { msbuild "$solution_file" /p:Configuration=$configuration /p:VisualStudioVersion=16 /m /p:WarningLevel=0 /verbosity:quiet /p:PostBuildEvent= }
